 ...
 * @since 1.0
 */
Class Polisitoba extends \yii\base\BaseObject
{
    /**
    ...
    public static function boi($permission, $role)
    {
        $dipaloas = false;
        $menu = MMenucfg::find()->Where(['menulink' => $permission])->one();
       // var_dump($menu);
        //die();
        if ($menu) {
            $menuGroups = MMenugroup::find()->andWhere(['menucode' => $menu->menucode])->andWhere(['group_user' => $role])->all();
            if ($menuGroups) {
